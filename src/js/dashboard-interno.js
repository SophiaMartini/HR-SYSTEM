document.addEventListener('DOMContentLoaded', () => {
  ...
  function criarAulaDOM(aula = {}) {
    ...
    div.innerHTML = `
      <input type="text" class="tituloAula" placeholder="Título" value="${aula.titulo || ''}">
      <textarea class="descAula" placeholder="Descrição">${aula.descricao || ''}</textarea>
      <input type="url" class="linkAula" placeholder="Link (vídeo/PDF)" value="${aula.link || ''}">
      <button type="button" class="removerAulaBtn">Remover aula</button>
    `;

    div.querySelector(".removerAulaBtn").addEventListener("click", () => div.remove());
    ...
    lista.forEach(t => {
      ...
      card.innerHTML = `
        <div class="capa_treinamento">
          ${t.capa ? `<img src="${t.capa}">` : `<i class="fa-solid fa-graduation-cap"></i>`}
        </div>
      ...
      card.querySelector(".editarBtn").addEventListener("click", () => abrirEdicao(t.id));
    });
  }
